...
export default function Home() {
  return (
    <main className="relative overflow-hidden">
      ...
      <section className="section container-wrap"><SectionHeader title="Built for ambitious operators" sub="Trusted by service businesses, ecommerce teams, startups, and local companies." /><div className="grid gap-5 md:grid-cols-3">{['Field service', 'Ecommerce', 'SMB teams'].map((x) => <TestimonialCard key={x} quote={`Built for ${x.toLowerCase()} to scale operations.`} who={x} />)}</div></section>
      <section className="section container-wrap"><SectionHeader title="What we build" sub="Custom software development, SaaS development, CRM systems for small business, and business automation software." /><div className="grid gap-5 md:grid-cols-2">{['Custom web apps', 'SaaS platforms', 'CRM systems', 'Automation workflows'].map((x) => <FeatureCard key={x} title={x} text="Production-grade systems focused on growth and efficiency." />)}</div></section>
      <section className="section container-wrap"><SectionHeader title="Featured product: HydroOps" sub="A modern operations platform for hydrovac and field service companies." /><ProductCard title="HydroOps" text="Scheduling, dispatch, job costing, reporting, and mobile workflows for field teams." /></section>
      <section className="section container-wrap"><SectionHeader title="Why Velora" sub="Founder-led product mindset, practical execution, and long-term partnership." /><div className="grid gap-5 md:grid-cols-3"><FeatureCard title="Practical" text="We solve business problems, not vanity features." /><FeatureCard title="Fast" text="Lean teams shipping high-quality code weekly." /><FeatureCard title="Reliable" text="Built with secure, scalable architecture." /></div></section>
      <section className="section container-wrap"><SectionHeader title="Process" sub="Discover, design, build, optimize." /><ol className="grid gap-4 md:grid-cols-4">{['Discovery', 'System Design', 'Build & Integrate', 'Scale & Support'].map((x) => <li key={x} className="card font-medium">{x}</li>)}</ol></section>
      <section className="section container-wrap"><SectionHeader title="Engagement models" sub="Flexible packages for each growth stage." /><div className="grid gap-5 md:grid-cols-3"><PricingCard title="Starter Build" text="Launch a focused product fast." /><PricingCard title="Growth System" text="Expand with automation and analytics." /><PricingCard title="Custom Platform" text="End-to-end platform for complex operations." /></div></section>
      <section className="section container-wrap"><SectionHeader title="FAQ" sub="Answers to common questions." /><FAQAccordion /></section>
      <section className="section container-wrap text-center"><h2 className="text-3xl font-semibold">Ready to build your next software advantage?</h2><p className="mt-4 text-slate-600 dark:text-slate-300">Talk with Velora about apps for field service companies, CRM modernization, and SaaS opportunities.</p><div className="mt-6"><Button href="/contact">Book a Demo</Button></div></section>
    </main>
  );
}
